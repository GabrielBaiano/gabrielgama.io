"use client";

import { useState, useEffect, useRef } from "react";
import { motion } from "framer-motion";
import { GitHubCalendar } from "react-github-calendar";

export function GithubHeatmapCard() {
    const [mounted, setMounted] = useState(false);
    const scrollContainerRef = useRef<HTMLDivElement>(null);

    useEffect(() => {
        const timer = setTimeout(() => {
            setMounted(true);
        }, 0);
        return () => clearTimeout(timer);
    }, []);

    // Auto-scroll to the end (most recent dates) when loaded
    useEffect(() => {
        if (mounted && scrollContainerRef.current) {
            // Small delay to ensure the calendar SVG has rendered its width
            const scrollTimer = setTimeout(() => {
                const container = scrollContainerRef.current;
                if (container) {
                    container.scrollLeft = container.scrollWidth;
                }
            }, 200);
            return () => clearTimeout(scrollTimer);
        }
    }, [mounted]);

    const item = {
        hidden: { opacity: 0, scale: 0.95, y: 20 },
        show: { opacity: 1, scale: 1, y: 0, transition: { type: "spring" as const, stiffness: 100, damping: 15 } }
    };

    return (
        <motion.div
            variants={item}
            className="col-span-2 md:col-span-4 lg:col-span-6 row-span-2 bg-[#0D1117] rounded-[24px] px-6 pt-5 pb-4 flex flex-col shadow-sm border border-[#30363D] relative overflow-hidden group"
        >
            <div className="flex flex-col mb-1 z-10 shrink-0">
                <div className="flex items-center gap-2 mb-1">
                    <svg
                        viewBox="0 0 24 24"
                        className="w-5 h-5 text-[#c9d1d9] fill-current"
                    >
                        <path d="M12 2C6.477 2 2 6.477 2 12c0 4.418 2.865 8.166 6.839 9.489.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.463-1.11-1.463-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0112 6.836c.85.004 1.705.114 2.504.336 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482C19.137 20.161 22 16.415 22 12c0-5.523-4.477-10-10-10z" />
                    </svg>
                    <span className="text-[12px] font-bold text-[#8b949e] uppercase tracking-[0.2em]">github contributions</span>
                </div>
            </div>

            <div className="flex-1 w-full flex items-center justify-center pt-2">
                <div className="w-full overflow-hidden">
                    <div
                        ref={scrollContainerRef}
                        className="max-w-full overflow-x-auto no-scrollbar scale-90 sm:scale-100 origin-bottom flex justify-center"
                    >
                        <div className="inline-block px-1">
                            {mounted && (
                                <GitHubCalendar
                                    username="GabrielBaiano"
                                    colorScheme="dark"
                                    theme={{
                                        dark: ['#161B22', '#0E4429', '#006D32', '#26A641', '#39D353'],
                                    }}
                                    showTotalCount={false}
                                    showColorLegend={false}
                                    showMonthLabels={false}
                                    labels={{
                                        months: [
                                            'Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun',
                                            'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'
                                        ],
                                        weekdays: ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'],
                                        totalCount: '{{count}} contribuições em {{year}}'
                                    }}
                                />
                            )}
                        </div>
                    </div>
                </div>
            </div>

            <style jsx global>{`
                .no-scrollbar::-webkit-scrollbar {
                    display: none !important;
                }
                .no-scrollbar {
                    -ms-overflow-style: none !important;  /* IE and Edge */
                    scrollbar-width: none !important;  /* Firefox */
                }
                .react-activity-calendar {
                    scrollbar-width: none !important;
                }
                .react-activity-calendar::-webkit-scrollbar {
                    display: none !important;
                }
            `}</style>
        </motion.div>
    );
}
